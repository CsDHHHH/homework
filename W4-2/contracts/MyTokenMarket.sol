...
interface MasterChef {
    ...
    struct PoolInfo {
        ...
        uint256 accCakePerShare; // Accumulated MDXs per share, times 1e12.
    ...
    function userInfo(uint256 pid, address user) external view returns (UserInfo memory _userInfo);
    function poolInfo(uint256 pid) external view returns (PoolInfo memory _poolInfo);
...
contract MyTokenMarket {
    ...
    function deposit(
        ...
        (uint _amtA, uint _amtB, uint _uniAmt) = _addLiquidity(tokenA,tokenB,_amt,unitAmt,slippage);

        uint256 balance = 
        MasterChef(masterchef).deposit(pid, balance);
    }
    ...
    function _withdrawSome(uint256 amount) internal returns (uint256 amountWithdraw) {
        amount = Math.min(amount, balanceOfPool());
        uint before = balanceOfWant();
        MasterChef(masterchef).withdraw(pid, amount);

        amountWithdraw = balanceOfWant().sub(before);
    }
