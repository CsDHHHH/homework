...
export async function routerCalc(web3, factoryAbi, routerAbi, tokenA, tokenB, amountIn, amountOut) {
    return (await routerCalcRaw(web3, factoryAbi, routerAbi, tokenA, tokenB, amountIn, amountOut))[0]
}

export async function priceAffect2(web3, tokenAbi, factoryAbi, routerAbi, tokenA, tokenB, amountA) {
    ...
    return [priceBefore, priceSwap, priceAfter]
}

// 参数 ： dsa, web3, depositToken, borrowToken, depositAmount
// const connectors = {
//     'lend': '',
//     'swap': '',
//     'swapFlash': ''
...
//     'fromComp':''
// }
async function baseLever(dsa, account, web3Instance, connectors, abis, methods, toCompToken, fromCompToken, toCompAmount, toCompAmountFact) {
    const spellsInner = dsa.current.Spell();
    const spells = dsa.current.Spell();

    const [path, _] = await routerCalc(web3Instance, abis.factory, abis.router, fromCompToken.address, toCompToken.address, 0, toCompAmountFact)
    const IdOne = 287875678;
    if (path.length == 2) {
        spellsInner.add({
            connector: connectors.lend,
            method: methods.toComp,
            args: [toCompToken.key, toCompAmount, 0, 0]
        })

        spellsInner.add({
            connector: connectors.lend,
            method: methods.fromComp,
            args: [fromCompToken.key, 0, IdOne, IdOne]
        })

        spellsInner.add({
            connector: connectors.swapFlash,
            method: "flashPayback",
    ...
    const transactionConfig = await dsa.current.getTransactionConfig(spellsInner)
    spells.add({
        connector: connectors.swapFlash,
        method: "flashBorrowAndCast",
        args: [
            toCompToken.cTokenAddress, //需要填对应的cToken的地址
            toCompToken.address,
            path[path.length - 2],
            toCompAmount,
            0, // route
            transactionConfig.data,
            IdOne
        ]
    });

    const txHash = await dsa.current.cast({
        spells,
        from: account
    });
    return txHash;
...
// 加杠杆
export async function lever(dsa, account, web3Instance, connectors, abis, depositToken, borrowToken, depositAmount) {
    return await baseLever(
        dsa,
        account,
        web3Instance,
        connectors,
...
// 减杠杆
export async function dislever(dsa, account, web3Instance, connectors, abis, paybackToken, withdrawToken, paybackAmount, paybackAmountFact) {
    return await baseLever(
        dsa,
        account,
        web3Instance,
...
// 更改担保物
export async function changeCollateral(dsa, account, web3Instance, connectors, abis, newCollateralToken, oldCollateralToken, newCollateralAmount, newCollateralAmountFact) {
    return await baseLever(
        dsa,
        account,
        web3Instance,
        ...
        oldCollateralToken,
        newCollateralAmount,
        newCollateralAmountFact
    )
}
// 更改债务
export async function changeDebt(dsa, account, web3Instance, connectors, abis, oldDebtToken, newDebtToken, oldDebtAmount, oldDebtAmountFact) {
    return await baseLever(
        dsa,
        account,
        web3Instance,
...
// 更改担保物-1处理
export async function changeCollateralSpecial(dsa, withdrawBalance, account, web3Instance, connectors, abis, toCompToken, fromCompToken, toCompAmount) {
    const spellsInner = dsa.current.Spell();
    const spells = dsa.current.Spell();

    const [path, _] = await routerCalc(web3Instance, abis.factory, abis.router, fromCompToken.address, toCompToken.address, 0, toCompAmount)

    //     const withdrawTokenContract = new  web3Instance.eth.Contract(abis.token, fromCompToken.address)
    const routerContract = new web3Instance.eth.Contract(abis.router, addrs.router)
    //     const withdrawBalance = await withdrawTokenContract.methods.balanceOf(dsa.address)
    const weigh = await routerContract.methods.getAmountsOut(withdrawBalance, path).call()

    spellsInner.add({
        connector: connectors['lend'],
        method: 'deposit',
        args: [toCompToken.key, weigh[weigh.length - 1], 0, 0]
    })
    spellsInner.add({
        connector: connectors['lend'],
        method: 'withdraw',
        args: [fromCompToken.key, weigh[0], 0, 0]
    })
    if (path.length > 2) {
        spellsInner.add({
            connector: connectors['swap'],
            method: "sellByPaths",
            args: [path.slice(0, path.length - 1), weigh[0], 0, 0]
        })
    }
    spellsInner.add({
        connector: connectors['swapFlash'],
        method: "flashPayback",
        args: [path[path.length - 2], weigh[weigh.length - 2], 0, 0],
    })
    ...
    spells.add({
        ...
        args: [
            toCompToken.cTokenAddress, //需要填对应的cToken的地址
            toCompToken.address, // 也就是path[path.length - 1]
            path[path.length - 2],
            weigh[weigh.length - 1],
            0, // route
            transactionConfig.data,
            0
        ]
